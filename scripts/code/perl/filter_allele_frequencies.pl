...
# validate init
GetOptions('vcffile=s', \$inputfilename, 'cutoff=f', \$cutoff, 'inverse', \$inverse);
...
while (<FILE>){
	...
	my $afsfield = (split ';', $info,3)[1]; # get AF field
	...
	if(! looks_like_number($afsvalue)){
		&handleMultiAllelic($line, $afsvalue);
		next;
	}
	...
	if($afslabel ne 'AF'){
		...
		if ((($cutoff < $afsvalue) && ! $inverse) || ($cutoff > $afsvalue && $inverse)){
			print STDOUT ("$linearray[0]\t$linearray[1]\t$linearray[2]\t$linearray[3]\t$linearray[4]\t$linearray[5]\t$linearray[6]\tZZ=Single;AF=$afsvalue;\n");
			$numlinesprinted++;
		}
		else{
			$numlinesomitted++;
...
sub handleMultiAllelic {
	...
	foreach(@alleles){
		if((($allelefreq[$int] > $cutoff) && ! $inverse) || ($cutoff > $allelefreq[$int] && $inverse)){
			print STDOUT ("$linearray[0]\t$linearray[1]\t$linearray[2]\t$linearray[3]\t$alleles[$int]\t$linearray[5]\t$linearray[6]\tZZ=Multiallelic;AF=$allelefreq[$int];\n");
		}
		$int++;
	}
	return 0;
