...
while (<FILE>){
	...
	if($afslabel ne 'AF'){
		...
		if (((($afscutoff < $afsvalue) && ! $inverse) || ($afscutoff > $afsvalue && $inverse)) && $acvalue >= $samplecutoff){
			print STDOUT ("$linearray[0]\t$linearray[1]\t$linearray[2]\t$linearray[3]\t$linearray[4]\t$linearray[5]\t$linearray[6]\tZZ=Single;AF=$afsvalue;AC=$acvalue\n");
			$numlinesprinted++;
		}
		else{
			$numlinesomitted++;
...
sub handleMultiAllelic {
	...
	foreach(@alleles){
		if(((($allelefreq[$int] > $afscutoff) && ! $inverse) || ($afscutoff > $allelefreq[$int] && $inverse)) && $acfreq[$int] >= $samplecutoff){
			print STDOUT ("$linearray[0]\t$linearray[1]\t$linearray[2]\t$linearray[3]\t$alleles[$int]\t$linearray[5]\t$linearray[6]\tZZ=Multiallelic;AF=$allelefreq[$int];AC=$acfreq[$int]\n");
			$numlinesprinted++;
		}
		else{
			$numlinesomitted++;
