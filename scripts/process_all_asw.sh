...
qsub -V -N align -b y -cwd -S /bin/sh -q all.q -pe make 2 -o "$i"_ASW_1.sai -e errorlog2.log ~/tools/bwa/bwa aln -R 100 -t 2 ~/new_ref_seq/alt_sequence2.fa "$i"_1.filt.fastq;
qsub -V -N align -b y -cwd -S /bin/sh -q all.q -pe make 2 -o "$i"_ASW_2.sai -e errorlog2.log ~/tools/bwa/bwa aln -R 100 -t 2 ~/new_ref_seq/alt_sequence2.fa "$i"_2.filt.fastq;
done
...
qsub -V -N sampe -b y -cwd -S /bin/sh -o "$i"_ASW_sampe.sam -e errorlog2.log ~/tools/bwa/bwa sampe -a 508 -P ~/new_ref_seq/alt_reference2.fa "$i"_ASW_1.sai "$i"_ASW_2.sai "$i"_1.filt.fastq "$i"_2.filt.fastq;
done
