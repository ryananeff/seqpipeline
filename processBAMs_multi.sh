...
#sambamba sort -t $threads "$name".bam -o "$name".sort.bam;
...
sambamba markdup -t $threads "$name".sort.bam "$name".rmdup.bam
...
if [ $? -ne 0 ]; then echo "$(date): exited with non-zero status ($?) during sambamba indexing post duplicate removal"; exit 1; else echo "$(date): indexing dup-rem BAM done."; fi
